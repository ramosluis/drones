...
#define terminal_baud_rate    230400                                  // terminal baud rate 115200
#define sf30_baud_rate        921600                                   // Sf30 baud rate  (Can be changed, refer to the sf30 manual)
...
void setup()                                          
{
  ...
  Serial1.begin(sf30_baud_rate);                                    // Open the second serial port to connect to the sf30
}

void loop()
{  
    Serial1.available();
    while (Serial1.available() > 0)                                 // Wait here for the next character
    {
    Byte_H = Serial1.read();                                        // Store the byte into Byte_H
    Byte_L = Serial1.read();                                        // Store the byte into Byte_L
    ...
    Serial1.available();                                            // Check buffer if byte available
    }
}
